...
def inicializar_draft(semana, tempo_por_escolha=None, modo_draft=None, max_times=None, max_jogadores_por_time=None):
    ...
    for i, time in enumerate(times):
        ...
        escolha_capitao = EscolhaDraft(
            ...
            time_id=time.id,
            ordem_escolha=i + 1,  # Primeiras escolhas são os capitães
            round_num=0,  # Rodada 0 para capitães
    ...
    draft_status = DraftStatus(
        ...
        rodada_atual=1,
        escolha_atual=len(times) + 1,  # Ajustado porque capitães já foram adicionados
        tempo_restante=semana.tempo_escolha,
        finalizado=False,
    ...
    if request.method == 'POST':
        ...
        if nome_completo:
            jogador_existente = Jogador.query.filter(
                func.lower(Jogador.nome) == func.lower(nome_completo),
                Jogador.ativo == True
            ).first()
        ...
        # Se jogador existir, vincula a conta
        if jogador_existente:
            if jogador_existente.user:
                flash('Este jogador já possui uma conta!', 'danger')
                return redirect(url_for('register'))
            
            jogador_id = jogador_existente.id
            role = 'capitao' if jogador_existente.capitao else 'jogador'
            
            # Atualiza dados do jogador se fornecidos
            if telefone:
...
def confirmar_presenca():
    ...
    # Se mensalista não confirmou, adiciona à lista de espera
    elif jogador.mensalista and not confirmar:
        ...
        if not lista_espera:
            nova_espera = ListaEspera(
                ...
                posicao_preferida=jogador.posicao,
                adicionado_em=datetime.utcnow()
            )
            db.session.add(nova_espera)
...
def admin_jogadores():
    jogadores = Jogador.query.filter_by(ativo=True).order_by(Jogador.nome).all()
    return render_template('admin/jogadores.html', jogadores=jogadores)
...
def promover_lista_espera(id):
    ...
    vagas_restantes = (semana.max_jogadores_por_time * semana.max_times) - total_confirmados
    
    if vagas_restantes <= 0:
        flash('Não há vagas disponíveis!', 'danger')
        return redirect(url_for('admin_dashboard'))
    
    # Busca jogador existente ou cria um novo
    jogador = Jogador.query.filter_by(nome=lista_espera.nome, ativo=True).first()
    ...
    db.session.commit()
    flash(f'{lista_espera.nome} promovido e adicionado aos confirmados!', 'success')
...
def capitao_escolher():
    ...
    draft_status.tempo_restante = semana.tempo_escolha  # CORREÇÃO: Reinicia o timer com configuração da semana
...
def visualizar_draft():
    ...
    return render_template('draft/publico.html',
                         ...
                         MAX_JOGADORES_POR_TIME=semana.max_jogadores_por_time)  # ADICIONAR ESTA LINHA
    ...
    if semana_id:
        ...
        if semana:
            ...
            emit('draft_status_update', {
                ...
                'tempo_restante': draft_status.tempo_restante if draft_status else TEMPO_ESCOLHA,
